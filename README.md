...
Allure был представлен на конференции «Тестовая среда» 30 ноября. Доклад был очень хорошо встречен публикой. Посмотреть доклад можно здесь [17], там же скачать презентацию. Сам проект можно найти в репозитории на github [18], сайт проекта [19] (пока только основа сайта, активно разрабатывается). По адресу [20]  можно посмотреть статистику проекта, а здесь [21] посмотреть, как выглядит отчет.
Фреймворк активно используется в нашем отделе тестирования, в основном используют адаптеры для JUnit, PyTest и TestNG.
Сейчас я занимаюсь тем, что пишу статью на Habrahabr, документацию и тесты, готовлю доклады на конференции. 
В принципе, поставленная задача с успехом выполнена. Проект обладает кроссязычностью, гибкостью и модульностью. Есть возможность заменять компонеты,  писать расширения. Например, почти сразу был написан плагин для Jenkins [22], позволяющий строить отчет без использования maven. 
