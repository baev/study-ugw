...
Этого достаточно, если речь идет о модульном тестировании (Unit testing). Но для более высокоуровневых тестов, например, проудктовых (Product testing), этого недостаточно. 

Такие тесты могут выполняться часы, и хочется уметь сохранять более подробную информацию о ходе тестов. Один из подходов для получения этой информации - разбиение теста на шаги (Steps). Также хотелось бы иметь возможность сохранять ...
